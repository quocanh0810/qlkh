...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $patternmakh = "/^[a-zA-Z0-9_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ]{3,10}$/";
    ...
    if(preg_match($patternmakh,$makh) == false){
        $flag = "false";
        $error['makh'] = "Mã khách hàng chỉ bao gồm chữ và số từ 3 đến 10 ký tự";
    }elseif($result_makh->num_rows > 0){
        $flag = "false";
        $error['makh'] = "Mã khách hàng đã tồn tại";
    }
    ...
    if($flag == "true"){
        $sql1 = "INSERT INTO khachhang (MAKH,TENKH,QuyMoDN,LinhVucKD,DiaChi,SDT,Email) VALUES ('$makh','$tenkh','$quymodn','$linhvuckd','$diachi','$sodienthoai','$email')";
